...
local vehicleFlyVelocityName = "VehicleFlyVelocity32"
local vehicleFlyGyroName = "VehicleFlyGyro64"
local vehicleFlyVelocity = nil
local vehicleFlyGyro = nil
local vehicleFlyConnection = nil
local vehicleFlyControl = {f = 0, b = 0, l = 0, r = 0}
...
function enableVehicleFly(speed)
    ...
    -- Clean up any leftover handlers from a previous session
    local existingVelocity = rootPart:FindFirstChild(vehicleFlyVelocityName)
    local existingGyro = rootPart:FindFirstChild(vehicleFlyGyroName)
    ...
    vehicleFlyVelocity.Name = vehicleFlyVelocityName
    vehicleFlyVelocity.MaxForce = Vector3.new(9e9, 9e9, 9e9)
    vehicleFlyVelocity.Velocity = Vector3.new(0, 0, 0)
    vehicleFlyVelocity.Parent = rootPart

    -- Create BodyGyro (pitch always ON)
    vehicleFlyGyro = Instance.new("BodyGyro")
    vehicleFlyGyro.Name = vehicleFlyGyroName
    vehicleFlyGyro.MaxTorque = Vector3.new(9e9, 9e9, 9e9)
    vehicleFlyGyro.P = 1000
    vehicleFlyGyro.D = 50
    vehicleFlyGyro.CFrame = rootPart.CFrame
    vehicleFlyGyro.Parent = rootPart

    -- Disconnect any old loop
    if vehicleFlyConnection then
        vehicleFlyConnection:Disconnect()
        vehicleFlyConnection = nil
    end

    -- Main vehicle fly RenderStepped loop
    vehicleFlyConnection = RunService.RenderStepped:Connect(function()
        ...
        local velHandler = root:FindFirstChild(vehicleFlyVelocityName)
        local gyroHandler = root:FindFirstChild(vehicleFlyGyroName)

        if not velHandler or not gyroHandler then
            return
        end

        local camera = workspace.CurrentCamera

        -- Pitch always ON: gyro fully tracks full camera CFrame including vertical angle
        gyroHandler.CFrame = camera.CFrame

        -- Build velocity from camera look/right vectors (pitch included since gyro is pitched)
        local camCF = camera.CFrame
        local vel = Vector3.new(0, 0, 0)

        if vehicleFlyControl.f ~= 0 then
            vel = vel + camCF.LookVector * vehicleFlyControl.f * 50 * vehicleFlySpeed
        end

        if vehicleFlyControl.b ~= 0 then
            vel = vel + camCF.LookVector * vehicleFlyControl.b * 50 * vehicleFlySpeed
        end

        if vehicleFlyControl.l ~= 0 then
            vel = vel + camCF.RightVector * vehicleFlyControl.l * 50 * vehicleFlySpeed
        end

        if vehicleFlyControl.r ~= 0 then
            vel = vel + camCF.RightVector * vehicleFlyControl.r * 50 * vehicleFlySpeed
        end

        velHandler.Velocity = vel

        -- If seated in a vehicle, push the vehicle along with the character
        local humanoid = char:FindFirstChild("Humanoid")
        if humanoid and humanoid.SeatPart then
            local seat = humanoid.SeatPart

            -- Try to find the vehicle's root/seat part to push
            local seatTarget = seat
            local parentModel = seat:FindFirstAncestorOfClass("Model")
            if parentModel then
                local vehicleSeat = parentModel:FindFirstChildOfClass("VehicleSeat")
                if vehicleSeat then
                    seatTarget = vehicleSeat
                end
            end

            -- Create or update the drive velocity on the vehicle
            local seatBodyVel = seatTarget:FindFirstChild("VFlyDriveVel")
            if not seatBodyVel then
                seatBodyVel = Instance.new("BodyVelocity")
                seatBodyVel.Name = "VFlyDriveVel"
                seatBodyVel.MaxForce = Vector3.new(9e9, 9e9, 9e9)
                seatBodyVel.Parent = seatTarget
            end

            seatBodyVel.Velocity = vel
        else
            -- Not seated, clean up any stray VFlyDriveVel instances
            for _, obj in pairs(workspace:GetDescendants()) do
                if obj.Name == "VFlyDriveVel" and obj:IsA("BodyVelocity") then
                    obj:Destroy()
                end
            end
        end
...
function disableVehicleFly()
    vehicleFlyEnabled = false
    vehicleFlyControl = {f = 0, b = 0, l = 0, r = 0}
    ...
    -- Destroy BodyVelocity handler
    if vehicleFlyVelocity then
        vehicleFlyVelocity:Destroy()
        vehicleFlyVelocity = nil
    end

    -- Destroy BodyGyro handler
    if vehicleFlyGyro then
        vehicleFlyGyro:Destroy()
        vehicleFlyGyro = nil
    end

    -- Also clean up directly on the character root just in case
    local char = LocalPlayer.Character
    if char then
        local root = char:FindFirstChild("HumanoidRootPart")
        if root then
            local leftoverVel = root:FindFirstChild(vehicleFlyVelocityName)
            local leftoverGyro = root:FindFirstChild(vehicleFlyGyroName)

            if leftoverVel then
                leftoverVel:Destroy()
            end

            if leftoverGyro then
                leftoverGyro:Destroy()
            end
        end
    end

    -- Remove any VFlyDriveVel instances left on vehicles in workspace
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj.Name == "VFlyDriveVel" and obj:IsA("BodyVelocity") then
            obj:Destroy()
        end
...
-- Vehicle Fly WASD Controls (separate from regular fly)
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed or not vehicleFlyEnabled then return end

    if input.KeyCode == Enum.KeyCode.W then
        vehicleFlyControl.f = 1
    elseif input.KeyCode == Enum.KeyCode.S then
        vehicleFlyControl.b = -1
    elseif input.KeyCode == Enum.KeyCode.A then
        vehicleFlyControl.l = -1
    elseif input.KeyCode == Enum.KeyCode.D then
        vehicleFlyControl.r = 1
    end
end)
...
UserInputService.InputEnded:Connect(function(input)
    ...
    if input.KeyCode == Enum.KeyCode.W then
        vehicleFlyControl.f = 0
    elseif input.KeyCode == Enum.KeyCode.S then
        vehicleFlyControl.b = 0
    elseif input.KeyCode == Enum.KeyCode.A then
        vehicleFlyControl.l = 0
    elseif input.KeyCode == Enum.KeyCode.D then
        vehicleFlyControl.r = 0
    end
end)
...
function closeRequireScriptsMenu()
    ...
    local titleBar = requireScriptsMenu:FindFirstChild("Frame")
    local scrollFrame = requireScriptsMenu:FindFirstChild("ScrollingFrame")
    ...
    -- Shrink animation
    local shrinkTween = TweenService:Create(requireScriptsMenu, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
...
function createChatLogger()
    if chatLogButton then
        createNotification("Chat logger already active!", false)
        return
    end
    ...
    chatLogButton.TextColor3 = Color3.fromRGB(0, 150, 255)
    ...
    buttonCorner.Parent = chatLogButton

    local buttonStroke = Instance.new("UIStroke")
    buttonStroke.Color = Color3.fromRGB(0, 100, 255)
    buttonStroke.Thickness = 2
    buttonStroke.Parent = chatLogButton

    -- Create main chat frame
    chatLogFrame = Instance.new("Frame")
    chatLogFrame.Name = "ChatLogFrame"
    chatLogFrame.Size = UDim2.new(0, 100, 0, 30)
    chatLogFrame.Position = UDim2.new(0, 10, 1, -40)
    chatLogFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    chatLogFrame.BorderSizePixel = 0
    chatLogFrame.Visible = false
    chatLogFrame.ClipsDescendants = true
    chatLogFrame.Active = true
    chatLogFrame.Draggable = true
    chatLogFrame.Parent = ScreenGui

    local frameCorner = Instance.new("UICorner")
    frameCorner.CornerRadius = UDim.new(0, 8)
    frameCorner.Parent = chatLogFrame

    local frameStroke = Instance.new("UIStroke")
    frameStroke.Color = Color3.fromRGB(0, 100, 255)
    frameStroke.Thickness = 2
    frameStroke.Parent = chatLogFrame

    -- Title
    local titleLabel = Instance.new("TextLabel")
    titleLabel.Name = "TitleLabel"
    titleLabel.Size = UDim2.new(0, 115, 0, 24)
    titleLabel.BackgroundTransparency = 1
    ...
    local function output(plr, msg)
        ...
        local colour = Color3.fromRGB(200, 200, 200) -- Grey for normal chat

        -- Check for command/whisper messages
    ...
    chatLogButton.MouseButton1Click:Connect(function()
        ...
        -- Expand animation
        local expandTween = TweenService:Create(chatLogFrame, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, 392, 0, 228)
        })
        expandTween:Play()
    ...
    logToggle.MouseButton1Click:Connect(function()
        ...
        logToggle.Text = chatLogging and "Log Chat [ON]" or "Log Chat [OFF]"
    end)
...
local Commands = {
    ...
    },
    {
        ...
        description = "Fly while in any vehicle or seat. Pitch is always ON. Use WASD to move.",
        requiresValue = true,
        valueType = "number",
        func = function(value)
            local speed = tonumber(value) or 1
            ...
            local ok, err = enableVehicleFly(speed)

            if ok == false then
                return false, err or "Failed to enable Vehicle Fly"
            end

            return true, "Vehicle Fly enabled | Speed: " .. speed .. " | Pitch: ON"
        end
    },
    {
        name = "√UnVehicleFly",
    ...
    },
    {
        ...
        description = "Enable chat logger",
        requiresValue = false,
        func = function()
            createChatLogger()
...
LocalPlayer.CharacterAdded:Connect(function(character)
    ...
    -- Re-enable vehicle fly if it was enabled
    if vehicleFlyEnabled then
        local savedSpeed = vehicleFlySpeed
        disableVehicleFly()
        task.wait(0.5)
        enableVehicleFly(savedSpeed)
    end
...
-- Monitor active states and update status bar
spawn(function()
    while true do
        if isOpen then
            ...
            if vehicleFlyEnabled then
                updateStatus("Status: Vehicle Fly Active (Speed " .. vehicleFlySpeed .. ") | Pitch: ON", Color3.fromRGB(0, 200, 255))
            elseif loopSpeedEnabled and loopJumpPowerEnabled then
...
print("[Encryption Admin] Vehicle Fly: √VehicleFly [speed] | Disable: √UnVehicleFly")
